...
class admin {
	...
	#use mysql for intermediate query generation in pages
	public $mysql_db = true;
	...
	public function build($post=array()) {
		...
		$this->filterString = array(
			...
			'audio'=>lang::translate('audio'), //'audio',
		);
		...
		switch($this->show) {
			case 'list':
				$this->addActionButtons(array('insert', 'home', 'filter', 'export', 'benchmark', 'multidelete'));
				$this->renderList();
				break;
			case 'insert':
				$this->addActionButtons(array('list', 'home', 'benchmark'));
				$this->renderInsert();
				break;
			case 'update':
				$this->addActionButtons(array('list', 'delete', 'home', 'insert', 'benchmark'));
				$this->renderUpdate();
				break;
			case 'delete':
				$this->makeDelete();
	...
	protected function makeAction() {
		...
		foreach($this->fields as $field=>$options) {
			...
			if(function_exists($options['validation'])){
			    $ret = array();  
				$func = $options['validation'];
				$ret = $func($this->POST[$field]);
				if(!$ret['success'])
				{
					$this->error[] = sprintf($ret['error'], $this->title(isset($options['label'])?$options['label']:$field));
				}
			}
			
			//compare with another field value
			if($options['compare']){
			//print_r($options['compare']);
				$val1 = $this->POST[$field];
				$val2 = $this->POST[$options['compare'][1]];
				$comparator = $options['compare'][1];
				$field2 = $options['compare'][0];
				switch($comparator){
					case '<':
						if(!($val1<$val2))
				$this->error[] = sprintf($options['label']." should be less than ".$field2 , $this->title(isset($options['label'])?$options['label']:$field));
				break;
				case '<=':
						if(!($val1<=$val2))
				$this->error[] = sprintf($options['label']." should be less than or equal to ".$field2 , $this->title(isset($options['label'])?$options['label']:$field));
				break;
				case '>':
						if(!($val1>$val2))
				$this->error[] = sprintf($options['label']." should be greater than ".$field2 , $this->title(isset($options['label'])?$options['label']:$field));
				break;
				case '>=':
						if(!($val1>=$val2))
				$this->error[] = sprintf($options['label']." should be greater than or equal to ".$field2 , $this->title(isset($options['label'])?$options['label']:$field));
				break;
				case '=':
						if(!($val1==$val2))
				$this->error[] = sprintf($options['label']." should be equal to ".$field2 , $this->title(isset($options['label'])?$options['label']:$field));
				break;
				case '!=':
						if(!($val1!=$val2))
				$this->error[] = sprintf($options['label']." should not be equal to ".$field2 , $this->title(isset($options['label'])?$options['label']:$field));
				break;
				}
		}
		
		//Compare with another field using custom validation
		if(isset($options["compare2field"])){
			$value1 = $this->POST[$field];
			$value2 = $this->POST[$options['compare2field'][0]];
			$func = $options['compare2field'][1];
			if(!function_exists($func)){
				$this->error[] = "Custom Validation Function doesn't exist";
			}
			else{
				$res = $func($value1,$value2);
				if(!$res['success'])
				{
					$this->error[] = sprintf($res['error'], $this->title(isset($options['label'])?$options['label']:$field));
				}
			...
			$this->mapping[] = array($options["type"],$options["identifier"], $count++);
		}
		// make INSERT or UPDATE - if no errors
		$sqli = mysql_query("SELECT * FROM $this->table");
		while($r = mysql_fetch_array($sqli)){
		$cnt = $r[$this->primaryKey];
		}
		$cnt = $cnt+1;
		if($this->captcha == true){
			if(!(isset($_SESSION['qaptcha_key']) && !empty($_SESSION['qaptcha_key'])))
		$this->error[] = lang::translate("captcha_is_not_validated");
		}
		if(empty($this->error)) {
		unset($_SESSION['qaptcha_key']);
			if($this->show == 'insert') {
				...
				$post_val = "";
				if($this->mapping[$i][0] == "groupCheckbox"){
				$post_val = $this->array_parse($this->POST[$this->mapping[$i][1]]);
				$sql2 = "UPDATE $this->table SET $fVal = '$post_val' WHERE $this->primaryKey = $cnt";
				//echo $sql2;
				array_push($this->queries, $sql2);
				}
				$i++;
				}
				...
				if($this->mapping[$i][0] == "groupCheckbox"){
				 $post_val = $this->array_parse($this->POST[$this->mapping[$i][1]]);
				$sql2 = "UPDATE $this->table SET $fVal = '$post_val' WHERE $this->primaryKey = $this->edit";
			...
			//run SQL for groupCheckbox
			foreach($this->queries as $v){
			//echo $v;
			mysql_query($v) or die("fcuk");
			}
			//Other postprocessing function	
			$func_post = (isset($this->postprocessing) AND function_exists($this->postprocessing)) ? $this->postprocessing :"";
	...
	protected function array_parse($arr){
	   $out = "";
		foreach($arr as $v){
			$out .= " ".$v;
		}
	...
	protected function validateFilter($field, $options) {
		...
		if($options["type"] == "groupCheckbox")
		if($this->POST[$options["identifier"]] == '' && isset($options['required']) && $options['required']) {
			return false;
		}
		// if no filter, all OK
			...
			for($iFiles = 1; $iFiles<=$multipleFiles; $iFiles++) {
				...
				if($value) {
					...
					switch($filter) {
						...
						case 'document':
							return in_array($ext, array('doc', 'docx', 'pdf', 'odf', 'xls', 'xlsx', 'ppt', 'pptx', 'odt', 'ods', 'xltx'));
						case 'archive':
							return in_array($ext, array('zip', 'rar', '7z', 'gzip', 'tar', 'gz'));
						case 'video':
							return in_array($ext, array('jpeg', 'flv', 'avi', 'mpg', 'mpeg', 'mov', 'mp4', '3gp', 'mkv', 'wmv', '3gpp', '3gpp2', 'mpeg4', 'rm' ));
						case 'audio':
							return in_array($ext, array('mp3', 'aac', 'm4a', 'midi', 'wav', 'wma'));
					}
				}
			}
		}
	...
	protected function addActionButtons($what) {
		...
		$templates = array(
			...
			'benchmark' => '<a target="_blank" href=benchmark.php?page=cont-'.$this->section.'>Benchmark Section</a>',
	...
	protected function input($field, $options) {
		...
		if(isset($options['keyboard']) && $options['keyboard'] != false)
		...
		$autocomplete = ($options["autocomplete"] == "off") ? "off" : "on";
		// check type
		switch($options['type']) {
			...
			case 'date':
				...
				break;
			case 'range':
			$def = $value ? $value : $options["default"];
			$iden = $options["identifier"];
				$out .= '<input type="range" name="'.$field.'" id="'.$field.'" value="'.$def.'" min="'.$options["min"].'" max="'.$options["max"].'" step="'.$options["step"].'" onchange="showvalue(this.value, '.$iden.')" style="width:'.$_f_width.'px" />&nbsp;&nbsp;<input type="text" id="'.$options['identifier'].'" value="'.$def.'" style="width:20px;"/>';
				break;
			...
			case 'checkbox':
				...
				break;
			case 'groupCheckbox':
				$a = $value ? explode(" ",$value) : "";
				foreach($options["select"] as $key=>$val){
				$sel = (in_array($key, $a)) ? "checked=checked" : "";
				$sel2 = (!$a AND isset($options["default"]) AND ($options["default"]==$key)) ? "checked=checked" : "";
				$out .= '<input type="checkbox" name="'.$options['identifier'].'[]" id="'.$field.'" value='.$key.' '.$sel.' '.$sel2.' />'.' '.$val.' ';
				}
				break;
			...
			case 'radio':
				foreach($this->getRadioField($options) as $selKey=>$selVal) {
					$out .= ' <input type="radio" name="'.$field.'" value="'.$this->h($selKey).'"'.($selKey == $value ? ' checked="checked"' : '').'/> '.$this->h($selVal);
				}
				break;
			...
			case 'password':
				$out = '<input type="password" name="'.$field.'" id="'.$field.'" value="'.$value.'" style="width:'.$_f_width.'px" class="'.$keyboard.'" onclick="this.select()" />';
				break;
			
			default:
			case 'order':
			case 'text':
				$out = '<input type="text" name="'.$field.'" id="'.$field.'" value="'.$value.'" autocomplete="'.$autocomplete.'" class="'.$keyboard.'" style="width:'.$_f_width.'px" />';
				break;
	...
	protected function renderList($export = false) {
		...
		foreach($record as $rec) {
			...
			foreach($this->fields as $k=>$v){
				if($v['hide'] != true)
				$this->fields_temp[$k] = $v;
			}
			foreach($this->fields_temp as $field=>$options) {
	...
	protected function renderCell($value, $options, $key=0) {
		...
		switch($options['type']) {
			...
			//group checkbox
			case 'groupCheckbox':
				$output = $this->getCheckboxField($options, $value);
				break;
			
			//radio
			case 'radio':
				$output = $this->getRadioField($options, $value);
				break;
			
			case 'range':
				$output = $this->getRangeField($options, $value);
				break;
		...
		if(isset($options['linkTo']) && is_array($options['linkTo']) && array_key_exists("show",$options['linkTo'])) {
			$urlToGo = $this->url($options['linkTo']);
		}
		elseif(isset($options['linkTo']) && is_array($options['linkTo']) && array_key_exists("extra",$options['linkTo']) && 		$options["linkTo"]["extra"] == "id=rowID"){
			$urlToGo = $this->url2($options['linkTo']).$key;
		}
		elseif(isset($options['linkTo']) && is_array($options['linkTo']) && array_key_exists("extra",$options['linkTo']) && 		$options["linkTo"]["extra"] != "id=rowID"){
			$urlToGo = $this->url2($options['linkTo']);			
		}
		if(isset($urlToGo) && ((array_key_exists("popup",$options['linkTo']) AND ($options['linkTo']['popup']=="true")))) {
		return '
		<div id="makeModalWindow" style="display:none"><iframe name="priviledge" style="width:920px;height:500px;border:0 none;" src='.$urlToGo.'></iframe></div>
	<div>
		<a href="#" onclick="return makeIframe(\'makeModalWindow\', { width:950, height:550, title:\''.$output.'\' })">'.$output.'</a>
	</div>';
	}
	if(isset($urlToGo) && ( (!array_key_exists("popup",$options['linkTo'])) )) {
		return "<a href='".$urlToGo."'>".$output."</a>";
		} 
		elseif(isset($urlToGo) &&  ($options['linkTo']['popup']==false))
		 {
			return "<a href='".$urlToGo."'>".$output."</a>";
		}
		else
	...
	public function getCheckboxField($options, $value=false) {
		$output = array();
		$output = $options['select'];
		//echo $output[$value];
		//echo $value;
	...
	public function getRadioField($options, $value=false) {
		$output = array();
			$output = $options['select'];
		// return out
		# if(count($output) == 1) { return current(array_values($output)); }
	...
	public function getRangeField($options, $value=false) {
		$output = array();
	...
	public function url2($replace=array(), $sign='&amp;', $flag = 2) {
		
		$out = "http://".$_SERVER["HTTP_HOST"].BASE_URL;
		foreach($replace as $k=>$v){
				if($k == "page"){
					$out.=$v;
				}
				if($k == "extra" && $v != "id=rowID"){
					$out.="?".$v;
				}
				elseif($k == "extra" AND $v == "id=rowID"){
					$v = str_ireplace("rowID", "", $v);
					$out.="?".$v;
					break;
				}
			}
